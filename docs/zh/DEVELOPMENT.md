...
- 基于关键词的命令安全检查（阻止 `rm`、`mkfs`、`dd` 等危险命令）
...
│   ├── security.rs         # SecurityChecker — 基于关键词的命令安全拦截
...
├── shell/                  # Shell 集成脚本
│   ├── install.sh          # 一键安装脚本
│   ├── remote-install.sh   # 远程 curl 安装脚本
│   ├── shellmate.bash      # Bash 集成
│   ├── shellmate.zsh       # Zsh 集成
...
- 使用 Rust 内置的 `#[test]` 属性，无外部测试框架
- 测试文件位于 `tests/` 目录，为集成测试
- 使用 `tempfile` crate 创建临时文件进行测试
- 测试命名：`test_<模块>_<行为>_<条件>`
- 使用 `assert!`、`assert_eq!` 和 `matches!()` 宏
...
- 从 `main` 创建特性分支
- 分支命名：`feat/feature-name`、`fix/bug-name`、`chore/task-name`
...
1. 确保所有测试通过：`cargo test`
2. 确保 lint 通过：`cargo clippy -- -D warnings`
3. 确保格式正确：`cargo fmt`
4. 创建带有清晰描述的 Pull Request
...
2. 运行 `cargo build --release` 验证构建
3. 运行全部测试：`cargo test`
4. 更新 `CHANGELOG.md`
5. 创建 git tag：`git tag v0.1.x`
6. 推送 tag：`git push origin v0.1.x`
7. CI 将自动构建并发布 Release 产物
...
查看 stderr 输出中的错误信息。如需更详细的调试，可以在对应 Provider 的 `chat_completion()` 方法中临时添加 `eprintln!` 输出请求/响应内容。
